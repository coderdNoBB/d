...
import com.example.demo.util.SimpleLoginFilter;
...
public class LoginResource {
	...
	@Autowired SimpleLoginFilter simpleLoginFilter;
	...
	public Response login (@RequestBody @Valid LoginRequest request,HttpServletRequest httpRequest) {
		if(request.getAccount().equals("d")) {
			if(!simpleLoginFilter.canLogin(request.getAccount(), request.getLoginFrom(), httpRequest)) {
				return Response.fail(Constants.LOGIN_NEED_VERIFOCATION);
			}else {
